...
def load_index_files():
    """Загрузка предварительно построенных файлов индекса"""
    index = {}
    ...
    with open('tfidf.csv', 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        doc_ids = next(reader)[1:]  # получаем список doc_id из заголовка
        for row in reader:
            term = row[0]
...
def main():
    morph = MorphAnalyzer()
    # Загрузка предварительно построенных индексов
